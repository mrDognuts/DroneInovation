...
from model_predictor import analyze_image
...
def capture_and_save_image(frame, output_dir="Dataset/before"):
    ...
    image_name = f"car{car_number}.png"
...
def automated_orbit_capture(tello, radius=50, steps=12, height=30):
    """Orbit around the car, capturing images at each step."""
    tello.takeoff()
    time.sleep(2)  # Allow time for takeoff

    tello.move_up(height)  # Set starting altitude
    time.sleep(2)
    ...
    for step in range(steps):
        ...
        analyze_image(image_path)  # Analyze the captured image

        # Move forward by a small distance and rotate
        tello.send_rc_control(0, radius, 0, 0)  # Move forward by radius distance
        time.sleep(1)  # Allow time for movement
        
        # Rotate to continue orbiting around the car
        tello.send_rc_control(0, 0, 0, 360 // steps)  # Rotate a fraction of 360 degrees
        time.sleep(1)  # Allow time for rotation

    tello.land()  # Land after completing the orbit
    print("Orbit completed and landed.")
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if 'o' in current_keys:  # Press 'o' to start the orbit automation
                automated_orbit_capture(tello)
